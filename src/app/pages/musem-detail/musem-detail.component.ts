    ...
    this.recorVisitService.getVisitsByMuseum(this.id).subscribe(
      result => {
        ...
        result.forEach(record => {
          //totals
          this.total_record += 1
          this.total_tourist += record.women!+record.men!
          this.total_visit += 1
  ...
  filterByDate() {
    ...
    this.recorVisitService.getVisitsByMuseum(this.id).subscribe(
      result => {
        ...
        this.average_visit=0
        this.period_visit.forEach(period=>{
          period.total=0
        })
        result.forEach(record => {
          //totals
          if (record.date!.toDate().getTime() >= new Date(this.start?.value).getTime() && record.date!.toDate().getTime() <= new Date(this.end?.value).getTime()) {
            console.log('entro')
            this.total_record += 1
            this.total_tourist += record.women!+record.men!
            this.total_visit += 1
